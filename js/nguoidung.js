...
window.onload = function () {
  ...
  if (currentUser) {
    ...
    var warning = `<h2 style="color: red; font-weight:bold; text-align:center; font-size: 2em; padding: 50px; margin-bottom:25%">
       Hãy đăng nhập/đăng kí để mua hàng !!!!!
                        </h2>`;
...
function addInfoUser(user) {
  ...
  document.getElementsByClassName("infoUser")[0].innerHTML =
    ...
    <table>
        <tr>
            <th style="color: red" colspan="3">THÔNG TIN KHÁCH HÀNG</th>
        </tr>
        <tr>
            <td>Tên tài khoản: </td>
...
`" readonly> </td>
    ...
    `₫" readonly> </td>
        ...
        <tr>
            <td>Tổng số lượng sản phẩm đã thanh toán: </td>
            <td> <input type="text" value="` +
...
function changePass() {
  ...
  if (inps[1].value.length < 4) {
    alert("Mật khẩu cần ít nhất 4 kí tự");
    return false;
  }
  ...
  if (!inp.readOnly && inp.value != "") {
    ...
    if (info == "username") {
      ...
      if (inp.value.length < 4) {
        alert("Tên đăng nhập cần ít nhất 4 kí tự");
